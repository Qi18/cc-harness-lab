"""s10 累计 Harness 的装配入口与父 Agent Loop。"""
...
# 上下文溢出只允许一次应急压缩重试，避免无界循环。
MAX_REACTIVE_RETRIES = 1


class AgentHarness:
    """组合 s01–s10 能力，并驱动父 Agent 的多轮工具循环。"""
    ...
    def __init__(self, client: OpenAI, settings: Settings):
        ...
        # 1. 创建有状态能力：Skill 注册表、Todo 状态和基础工具 handler。
    ...
    def agent_loop(
        ...
        while True:
            ...
            # Provider 统一补齐模型专属参数，Agent Loop 只关心 messages 和 tools。
            try:
                response = self.client.chat.completions.create(
                    **completion_request(self.settings, messages, tools)
                )
                reactive_retries = 0
            except Exception as exc:
                # 只有明确的上下文溢出才走 reactive compact；其他异常原样抛出。
                if (
            ...
            # assistant 消息必须先完整写回历史，后续 role=tool 才能按 ID 配对。
