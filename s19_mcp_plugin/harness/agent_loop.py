"""s19 累计 Harness 装配：Lead 与自治队友共享任务板。"""
...
from . import memory
from .agent_teams import AgentTeamManager
...
from .mcp_plugin import MCPManager
...
class AgentHarness:
    """组合 s01–s19 能力，并驱动后台任务、定时任务与 Agent Teams。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.worktrees = WorktreeManager(settings, self.tasks)
        ...
        self._parent_handlers: dict[str, Callable[..., str]] = {
            **self.builtins.handlers(),
            "connect_mcp": self._connect_mcp,
            **self.tasks.handlers(),
            **self.worktrees.handlers(),
            **self.cron.handlers(),
            **self.team.handlers(),
    ...
    def refresh_system_prompts(
        ...
        self.system_prompt = self.parent_prompt.get(
            self._prompt_context(self._visible_parent_tools(False))
        )
    ...
    def _connect_mcp(self, name: str) -> str:
        # 连接校验包含内置 schema 与 handler，失败不发布半成品工具池。
        result = self.mcp.connect_mcp(name, PARENT_TOOLS, self._parent_handlers)
        # Prompt 缓存键已包含工具名，因此工具新增会自然失效，无需删除缓存机制。
        self.refresh_system_prompts()
        return result
    ...
    def _dispatch_parent_tool(self, tool_call: Any) -> str:
        ...
        # 每次分发读取最新快照；同一响应批次先连接、后调用也能正确路由。
        _, handlers = self.mcp.assemble_tool_pool(PARENT_TOOLS, self._parent_handlers)
    ...
    def _inject_team_messages(
        self,
        messages: list[dict[str, Any]],
        extraction_messages: list[dict[str, Any]],
    ) -> int:
        """把 Lead 邮箱作为独立 user 事件注入，不混入工具协议消息。"""

        try:
            inbox = self.team.consume_lead_messages()
        except Exception as error:
            print(f"\033[31m[team inbox failed] {error}\033[0m")
            return 0
        if not inbox:
            return 0
        item = self.team.inbox_event(inbox)
        messages.append(item)
        extraction_messages.append(copy.deepcopy(item))
        senders = ", ".join(
            sorted({message.from_agent for message in inbox})
        )
        print(
            "\033[35m[team inbox] "
            f"{len(inbox)} message(s) from {senders}\033[0m"
        )
        return len(inbox)
    ...
    def agent_loop(
        ...
        while True:
            ...
            if not assistant.tool_calls:
                ...
                team_count = self._inject_team_messages(
                    messages, extraction_messages
                )
                if background_count or team_count:
                    continue
