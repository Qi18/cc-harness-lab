...
from .background import ShellExecutor, format_shell_result
...
BASH_TOOL = function_tool(
    "bash",
    (
        "Run a Bash command in the agent working directory. Set "
        "run_in_background=true only for an independent slow command whose "
        "result is not needed by the next action."
    ),
    {
        "command": {"type": "string"},
...
SUB_TOOLS = [SUB_BASH_TOOL, *BASE_TOOLS[1:], LOAD_SKILL_TOOL]
...
class BuiltinTools:
    ...
    def run_bash(self, command: str, run_in_background: bool = False) -> str:
        """Run synchronously; AgentHarness intercepts an explicit background flag."""
        del run_in_background
        return format_shell_result(*self.shell.run(command))
