...
from s15_agent_teams.harness import memory
from s15_agent_teams.harness.background_tasks import (
    COMPLETED,
    FAILED,
    BackgroundTaskManager,
)
from s15_agent_teams.harness.agent_loop import AgentHarness
from s15_agent_teams.harness.agent_teams import (
    TEAMMATE_TOOLS,
    AgentTeamManager,
    MessageBus,
    TeamMessage,
)
from s15_agent_teams.harness.config import Settings
from s15_agent_teams.harness.cron_scheduler import (
    CronJob,
    CronScheduler,
    cron_matches,
    validate_cron,
)
from s15_agent_teams.harness.error_recovery import (
    CONTINUATION_PROMPT,
    DEFAULT_MAX_TOKENS,
...
from s15_agent_teams.harness.subagent import SubagentRunner
from s15_agent_teams.harness.system_prompt import (
    SystemPromptAssembler,
    registered_tool_names,
)
from s15_agent_teams.harness.task_system import TaskManager
from s15_agent_teams.harness.todo_write import TodoManager
from s15_agent_teams.harness.tool_use import PARENT_TOOLS, SUB_TOOLS, ToolExecutor
...
class SystemPromptTest(unittest.TestCase):
    ...
    def test_tool_section_comes_from_real_registries(self):
        ...
        self.assertEqual(20, len(parent_names))
    ...
    def test_modules_follow_course_capability_names(self):
        harness_dir = REPO_ROOT / "s15_agent_teams" / "harness"
...
class AgentTeamsTest(unittest.TestCase):
    ...
    def test_team_tools_are_parent_only_and_teammate_tools_are_minimal(self):
        ...
        self.assertEqual(
            ...
            [name for name in parent_names if name in {
                "spawn_teammate", "send_message", "check_inbox"
            }],
        )
        self.assertTrue(
            {"spawn_teammate", "send_message", "check_inbox"}.isdisjoint(
                child_names
            )
        )
        self.assertEqual(
            ["bash", "read_file", "write_file", "send_message"],
            teammate_names,
        )
        ...
        self.assertIn("spawn_teammate", harness.system_prompt)
