...
from s19_mcp_plugin.harness import memory
from s19_mcp_plugin.harness.background_tasks import (
    COMPLETED,
    FAILED,
    BackgroundTaskManager,
)
from s19_mcp_plugin.harness.agent_loop import AgentHarness
from s19_mcp_plugin.harness.agent_teams import (
    TEAMMATE_TOOLS,
    AgentTeamManager,
    MessageBus,
    TeamMessage,
)
from s19_mcp_plugin.harness.config import Settings
from s19_mcp_plugin.harness.cron_scheduler import (
    CronJob,
    CronScheduler,
    cron_matches,
    validate_cron,
)
from s19_mcp_plugin.harness.error_recovery import (
    CONTINUATION_PROMPT,
    DEFAULT_MAX_TOKENS,
...
from s19_mcp_plugin.harness.subagent import SubagentRunner
from s19_mcp_plugin.harness.system_prompt import (
    SystemPromptAssembler,
    registered_tool_names,
)
from s19_mcp_plugin.harness.task_system import TaskManager
from s19_mcp_plugin.harness.team_protocols import TeamProtocolManager
from s19_mcp_plugin.harness.todo_write import TodoManager
from s19_mcp_plugin.harness.tool_use import PARENT_TOOLS, SUB_TOOLS, ToolExecutor
...
class SystemPromptTest(unittest.TestCase):
    ...
    def test_tool_section_comes_from_real_registries(self):
        ...
        self.assertEqual(27, len(parent_names))
    ...
    def test_modules_follow_course_capability_names(self):
        harness_dir = REPO_ROOT / "s19_mcp_plugin" / "harness"
...
class AutonomousAgentsTest(unittest.TestCase):
    ...
    def test_idle_prioritizes_mail_over_claiming(self):
        from s19_mcp_plugin.harness.autonomous_agents import AutonomousWorker
...
class WorktreeIsolationTest(unittest.TestCase):
    def setUp(self):
        ...
        from s19_mcp_plugin.harness.worktree_isolation import WorktreeManager
    ...
    def test_name_validation_and_existing_branch_are_non_destructive(self):
        from s19_mcp_plugin.harness.worktree_isolation import validate_worktree_name
    ...
    def test_context_manual_claim_and_unbound_reset(self):
        from s19_mcp_plugin.harness.worktree_isolation import WorktreeContext
        client, _ = fake_client()
...
class MCPPluginTest(unittest.TestCase):
    """验证动态 schema、实际路由和教学 mock 的边界，而非真实传输层。"""

    def setUp(self):
        from s19_mcp_plugin.harness.mcp_plugin import MCPManager
        self.temporary = tempfile.TemporaryDirectory()
    ...
    def harness(self, *responses):
        settings = SystemPromptTest.settings(self)
        client, completions = fake_client(*responses)
        return AgentHarness(client, settings), completions

    @staticmethod
    def call(tool_name, **args):
        return SimpleNamespace(id=tool_name, function=SimpleNamespace(
            name=tool_name, arguments=json.dumps(args)))

    @staticmethod
    def factory(server, names):
        ...
        def make():
            ...
            client.register([
                ...
                for n in names
            ], {n: (lambda n=n: n) for n in names})
            return client
        return make

    def test_connect_discovers_openai_schema_and_annotations(self):
        self.assertIn("Connected", self.manager.connect_mcp("docs"))
        tools, handlers = self.manager.assemble_tool_pool([], {})
        self.assertEqual(2, len(tools))
        self.assertEqual(set(handlers), {t["function"]["name"] for t in tools})
        search = tools[0]["function"]
        self.assertIn("(readOnly)", search["description"])
        self.assertEqual(["query"], search["parameters"]["required"])
        self.assertEqual("worktree", json.loads(handlers[search["name"]](query="worktree"))["results"][0]["id"])

    def test_duplicate_and_unknown_connections_do_not_change_pool(self):
        self.assertIn("Unknown", self.manager.connect_mcp("external-command"))
        self.manager.connect_mcp("docs")
        before = self.manager.assemble_tool_pool([], {})[0]
        self.assertIn("already connected", self.manager.connect_mcp("docs"))
        self.assertEqual(before, self.manager.assemble_tool_pool([], {})[0])
        self.assertIn("MCP error", self.manager.connect_mcp(None))

    def test_two_servers_route_to_original_handlers_without_late_binding(self):
        self.manager.connect_mcp("docs")
        self.manager.connect_mcp("deploy")
        tools, handlers = self.manager.assemble_tool_pool([], {})
        self.assertEqual(4, len(tools))
        self.assertEqual("worktree", json.loads(handlers["mcp__docs__get"](document_id="worktree"))["id"])
        self.assertEqual("not_started", json.loads(handlers["mcp__deploy__status"](service="api"))["status"])
        self.assertEqual("simulated", json.loads(handlers["mcp__deploy__deploy"](service="api"))["status"])
        self.assertEqual("simulated", json.loads(handlers["mcp__deploy__status"](service="api"))["status"])
        self.assertIn("(destructive)", next(t for t in tools if t["function"]["name"]=="mcp__deploy__deploy")["function"]["description"])

    def test_normalized_server_collision_is_rejected(self):
        from s19_mcp_plugin.harness.mcp_plugin import MCPManager
        manager = MCPManager({n:self.factory(n, ["read"]) for n in ("a.b", "a/b")})
        self.assertIn("Connected", manager.connect_mcp("a.b"))
        self.assertIn("collision", manager.connect_mcp("a/b"))
        self.assertEqual(1, len(manager.assemble_tool_pool([], {})[0]))

    def test_normalized_tool_collision_is_atomic(self):
        from s19_mcp_plugin.harness.mcp_plugin import MCPManager
        manager = MCPManager({"docs":self.factory("docs", ["a.b", "a/b"])})
        self.assertIn("collision", manager.connect_mcp("docs"))
        self.assertEqual(([], {}), manager.assemble_tool_pool([], {}))

    def test_oversized_tool_name_is_rejected(self):
        from s19_mcp_plugin.harness.mcp_plugin import MCPManager
        manager = MCPManager({"docs":self.factory("docs", ["a"*65])})
        self.assertIn("64", manager.connect_mcp("docs"))
        self.assertEqual(([], {}), manager.assemble_tool_pool([], {}))

    def test_builtin_handler_collision_never_overwrites(self):
        existing = {"mcp__docs__search":lambda: "builtin"}
        self.assertIn("collision", self.manager.connect_mcp("docs", [], existing))
        self.assertEqual(([], {}), self.manager.assemble_tool_pool([], {}))
        self.assertEqual("builtin", existing["mcp__docs__search"]())

    def test_client_rejects_invalid_registration_and_reports_call_errors(self):
        from s19_mcp_plugin.harness.mcp_plugin import MCPClient
        client = MCPClient("test")
        with self.assertRaises(ValueError):
            client.register([{"name":"bad", "inputSchema":{"type":"array"}}], {"bad":lambda:None})
        self.assertEqual([], client.tools)
        client = self.factory("test", ["read"])()
        self.assertIn("MCP error", client.call_tool("missing", {}))
        self.assertIn("MCP error", client.call_tool("read", []))
        self.assertIn("MCP error", client.call_tool("read", {"unexpected":True}))
        snapshot = client.tools
        snapshot[0]["name"] = "changed"
        self.assertEqual("read", client.tools[0]["name"])

    def test_handler_exception_is_a_tool_result(self):
        from s19_mcp_plugin.harness.mcp_plugin import MCPClient
        client = MCPClient("broken")
        def fail():
            raise RuntimeError("intentional")
        client.register([{"name":"fail","inputSchema":{"type":"object"}}], {"fail":fail})
        self.assertIn("RuntimeError: intentional", client.call_tool("fail", {}))

    def test_factory_failure_keeps_existing_server_usable(self):
        def broken():
            raise RuntimeError("offline")
        self.manager.factories["broken"] = broken
        self.manager.connect_mcp("docs")
        self.assertIn("offline", self.manager.connect_mcp("broken"))
        self.assertEqual(2, len(self.manager.assemble_tool_pool([], {})[0]))

    def test_parallel_connect_publishes_each_server_once(self):
        threads=[threading.Thread(target=self.manager.connect_mcp,args=(n,))
                 for n in ("docs","docs","deploy","deploy")]
        for t in threads:t.start()
        for t in threads:t.join(timeout=2)
        self.assertTrue(all(not t.is_alive() for t in threads))
        self.assertEqual(4, len(self.manager.assemble_tool_pool([], {})[0]))

    def test_pool_returns_copies_and_keeps_builtin_order(self):
        builtin = [{"type":"function","function":{"name":"local","parameters":{"type":"object"}}}]
        self.manager.connect_mcp("docs")
        tools, _ = self.manager.assemble_tool_pool(builtin, {})
        self.assertEqual("local", tools[0]["function"]["name"])
        tools[0]["function"]["name"]="changed"
        tools[1]["function"]["parameters"]["required"]=[]
        again, _ = self.manager.assemble_tool_pool(builtin, {})
        self.assertEqual("local", again[0]["function"]["name"])
        self.assertEqual(["query"], again[1]["function"]["parameters"]["required"])

    def test_dynamic_prompt_cache_and_compact_filter(self):
        h,_=self.harness()
        before=h.parent_prompt.assembly_count
        h._dispatch_parent_tool(self.call("connect_mcp",name="docs"))
        self.assertGreater(h.parent_prompt.assembly_count,before)
        self.assertIn("mcp__docs__search",h.system_prompt)
        self.assertNotIn("mcp__docs__",h.sub_system_prompt)
        tools=h._visible_parent_tools(True)
        names={t["function"]["name"] for t in tools}
        self.assertIn("mcp__docs__search",names)
        self.assertNotIn("compact",names)
        count=h.parent_prompt.assembly_count
        h.refresh_system_prompts()
        self.assertEqual(count,h.parent_prompt.assembly_count)

    def test_teammate_and_subagent_cannot_dispatch_mcp(self):
        h,_=self.harness()
        h._connect_mcp("docs")
        for handlers in (h.subagent.handlers,h.team._teammate_handlers("alice")):
            result=h.executor.execute("mcp__docs__search",'{"query":"mcp"}',handlers)
            self.assertIn("unknown tool",result)
        self.assertEqual(8,len(TEAMMATE_TOOLS))
        self.assertEqual(6,len(SUB_TOOLS))

    def test_full_loop_connect_then_call_then_answer(self):
        from unittest.mock import patch
        h,completions=self.harness(
            FakeChoice(None,tool_calls=[self.call("connect_mcp",name="docs")]),
            FakeChoice(None,tool_calls=[self.call("mcp__docs__search",query="worktree")]),
            "Found worktree documentation.",
        )
        messages=[{"role":"user","content":"Connect docs and search worktree."}]
        with patch.object(memory,"load_memories",return_value=[]), patch.object(memory,"extract_memories"):
            answer=h.agent_loop(messages,compactor=IdentityCompactor())
        self.assertEqual("Found worktree documentation.",answer)
        first,second,third=completions.requests
        self.assertNotIn("mcp__docs__search",{t["function"]["name"] for t in first["tools"]})
        self.assertIn("mcp__docs__search",{t["function"]["name"] for t in second["tools"]})
        self.assertIn("mcp__docs__search",second["messages"][0]["content"])
        results=[m for m in third["messages"] if m["role"]=="tool"]
        self.assertEqual(2,len(results))
        self.assertEqual("worktree",json.loads(results[-1]["content"])["results"][0]["id"])

    def test_same_batch_connect_and_call_resolves_latest_pool(self):
        h,_=self.harness()
        self.assertIn("Connected",h._dispatch_parent_tool(self.call("connect_mcp",name="docs")))
        result=h._dispatch_parent_tool(self.call("mcp__docs__get",document_id="mcp"))
        self.assertEqual("mcp",json.loads(result)["id"])
