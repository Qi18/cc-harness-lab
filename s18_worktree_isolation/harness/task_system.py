...
class TaskRecord:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "TaskRecord":
        ...
        if worktree is not None and (
            ...
            or not re.fullmatch(r"[A-Za-z0-9_][A-Za-z0-9._-]{0,63}", worktree)
            or ".." in worktree or worktree.endswith((".", ".lock"))
        ):
            raise TaskError("invalid task worktree")
...
class TaskManager:
    ...
    @board_locked
    def bind_task_to_worktree(self, task_id: str, name: str) -> None:
        """绑定不修改 status/owner；一项任务和一个目录相互独占。"""

        task = self._load(task_id)
        if task.status != PENDING:
            raise TaskError("only pending tasks can be bound")
        if task.worktree is not None:
            raise TaskError("task is already bound")
        if any(t.worktree == name for t in self._all()):
            raise TaskError("worktree is already bound to another task")
        bound = replace(task, worktree=name)
        TaskRecord.from_dict(bound.to_dict())
        self._save(bound)

    @board_locked
    def unbind_worktree(self, name: str) -> None:
        """清理目录关联，保留任务完成状态和原 owner。"""

        for task in self._all():
            if task.worktree == name:
                self._save(replace(task, worktree=None))
